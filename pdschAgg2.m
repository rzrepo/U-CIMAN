...
if exist(fullfile(cd, 'dlrbcellprocessed.mat'), 'file')
    ...
    save('dlrbcellprocessed.mat', '-v7.3');
end
...
for i = 1:24
    ...
    current_rb_per_tti = sum(currentH>0);
    mean_rb_per_tti_per_hour(i) = mean(current_rb_per_tti); % average number of rb per tti
    pctileLow_rb_per_tti_per_hour(i) = prctile(current_rb_per_tti, 20); % 15 percentile of rb per tti
    pctileHigh_rb_per_tti_per_hour(i) = prctile(current_rb_per_tti, 80); % 85 percentile of rb per tti
    min_rb_per_tti_per_hour(i) = prctile(current_rb_per_tti, 1); % min rb per tti
    max_rb_per_tti_per_hour(i) = prctile(current_rb_per_tti, 99); % max rb per tti
end

f = figure('rend','painters','pos',[100 100 600 300]);
colormap copper;
cmap = colormap;
newcolors = cmap(15:64,:);
set(f, 'defaultAxesColorOrder', newcolors);
set(0,'defaultAxesFontName', 'Arial');
hold on; grid on;
bar(0:2, per_tti_nrb_record([4 8 16],:), 'EdgeColor','white', 'LineWidth',0.1);
colorbar;
set(gca, 'YScale', 'log');
...
set(0,'defaultAxesFontName', 'Arial');
hold on; grid on;
bar([0 0.8 1.6], hour_rb_pos_count([4 8 16],:), 'EdgeColor','white', 'LineWidth',0.1);
% colorbar;
xlabel('Hour of the day');
