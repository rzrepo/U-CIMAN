 function pdschAgg3(name)
% analyze pdsch cell staticstics
   name_postfix = name;
% name_postfix = '24';

    if exist(fullfile(cd, ['dlrbcellprocessed' name_postfix '.mat']), 'file')
        load(['dlrbcellprocessed' name_postfix '.mat']);
    else
        ...
        save(['dlrbcellprocessed' name_postfix '.mat'], '-v7.3');
    end
    ...
    print(fig, ['DL_RB_size_dist' name_postfix],'-dpdf');
    savefig(['DL_RB_size_dist' name_postfix]);
    ...
    fig.PaperSize = [fig_pos(3) fig_pos(4)];
    set(findall(gcf,'-property','FontSize'),'FontSize',14);
    print(fig,['dltotalrbusage' name_postfix],'-dpdf');
    savefig(['dltotalrbusage' name_postfix]);
 end