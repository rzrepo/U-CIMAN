function usage_out = get_usage_mcs(ra_indicator, mcs, is_down, usage_input)
    ...
    if ra_type == 2
        ...
        if rb_start+length-1 > nrb
            ...
            usage = [-ones(1, rb_start-1) ones(1, length).*mcs -ones(1, nrb - (rb_start + length -1))];
            temp_usage_out = (usage==-1).*temp_usage_in + (usage~=-1).*usage;
        end
    elseif ra_type == 0
        if (nrb <=10 )
            p = 1;
        ...
        if rbg_bitmask>2^nof_bit-1
            ...
            usage = double([repelem(usage(1:end-1), p) repelem(usage(end), last_bit_repeat_times)].*(mcs+1))-1;
            usage = double(usage);
            temp_usage_out = (usage==-1).*temp_usage_in + (usage~=-1).*usage;
        end
    elseif ra_type == 1
        ...
        if vrb_bitmask>2^nof_bitmask-1
            ...
            current_usage = double(current_usage.*(mcs+1))-1;
            temp_usage_out = (current_usage==-1).*temp_usage_in + (current_usage~=-1).*current_usage;
        end
    else
        temp_usage_out = temp_usage_in;
    end
