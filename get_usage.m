function usage_out = get_usage(ra_indicator, is_down, usage_input)
    usage_out = usage_input;
    nrb = (size(usage_input, 2)-1)/2;
    if (is_down)
        temp_usage_in = usage_input(2:(nrb+1));
    else
        temp_usage_in = usage_input((end-nrb+1):end);
    end
    ...
    if ra_type == 2
        ...
        if rb_start+length-1 > nrb
            ...
            usage = [zeros(1, rb_start-1) ones(1, length) zeros(1, nrb - (rb_start + length -1))];
            temp_usage_out = or(usage, temp_usage_in);
        end
    elseif ra_type == 0
        if (nrb <=10 )
            p = 1;
        ...
        if rbg_bitmask>2^nof_bit-1
            ...
            usage = [repelem(usage(1:end-1), p) repelem(usage(end), last_bit_repeat_times)];
            temp_usage_out = or(usage, temp_usage_in);
        end
    elseif ra_type == 1
        if (nrb <=10 )
        ...
        if vrb_bitmask>2^nof_bitmask-1
            ...
            temp_usage_out = or(current_usage, temp_usage_in);
        end
    else
        temp_usage_out = temp_usage_in;
    end
    if (is_down)
        ...
        usage_out((end-nrb+1):end) = temp_usage_out;
    end
end